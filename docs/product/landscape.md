...
**Non-overlap.** MyChatArchive imports ChatGPT, Claude, Grok, Claude Code terminal sessions, and Cursor IDE sessions. SoulPrint imports ChatGPT, Claude, Claude Code, Gemini, and Grok. Only SoulPrint supports Gemini exports (Takeout `MyActivity.json` and Chrome-extension conversational JSON). Neither currently supports CharacterAI; both see it as a natural next target.
