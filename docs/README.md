# SoulPrint Documentation

## Getting Started
- [Setup and first run](getting-started.md)

## Product
- [Manifesto](manifesto.md)
- [Positioning](product/positioning.md)
- [Landscape](product/landscape.md)
- [Brand guide](product/brand.md)
- [Visual direction](product/visual-direction.md)

## Architecture
- [Answering boundary](architecture/answering-boundary.md)
- [Retrieval surface](architecture/retrieval-surface.md)
- [mem0 boundary](architecture/mem0-boundary.md) *(optional, not active)*

## Specs
- [Memory Passport v1](specs/memory-passport-spec.md)
- [Obsidian Bridge](specs/obsidian-bridge-spec.md)
- [Intent Prompts v1](specs/intent-prompts-spec.md)
- [Frontend Evolution Doctrine](specs/frontend-evolution-doctrine.md)
- [Session Attachments](specs/session-attachments-spec.md)
- [Tagging](specs/tagging-spec.md)
- [Companion Layer Plan](specs/soulprint-companion-layer-plan.md) *(partially shipped)*
- [Expansion Plan](specs/soulprint-expansion-plan.md) *(partially shipped)*

## Guides
- [Obsidian raw inbox bridge](obsidian-raw-inbox.md): feeding a Kepano-style vault

## Reference
- [Project status (2026-03-09)](reference/history/project-status-2026-03-09.md)
- [Repair notes](reference/history/repair-notes.md)
- [Repo audit baseline](reference/history/repo-audit-baseline.md)

## Releases
- [v0.1.0 release notes](releases/RELEASE-v0.1.0.md)

## Archive

Frozen artifacts from retired design systems and superseded execution plans. See [`archive/`](archive/).