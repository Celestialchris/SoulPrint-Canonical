...
- **`soulprint verify` subcommand.** Deterministic health checks against the canonical SQLite ledger: DB existence, SQLite integrity, core tables, FTS tables, and orphan messages. Human and `--json` output. Exit 0 healthy / 1 check failed / 2 DB missing. Backed by reusable `src/verify.py` module with a fixed return shape that PR next-3 (`/archive/health`) will consume directly.
- **Starred imports on `/chats`.** Imported conversations with `is_starred=True` now appear below the notes list on "Your own notes", with title link, provider pill, read-only tag chips, timestamp, and an Unstar action. Starring an import now has a curated destination. No schema changes; query-and-template extension only.
- **Tags on imported conversations (MVP).** Every imported conversation now gets a first-meaningful-word auto-tag extracted from its title at import time (with a small stopword filter for words like "the", "how", "my"). Tags render as inline chips on `/imported` list rows, editable via plain form POSTs (add input at row end, remove button per chip) — no JavaScript required. Schema is a comma-separated string on `ImportedConversation.tags`, matching the existing `MemoryEntry.tags` convention. New `src/app/tags.py` module centralizes normalization (lowercase, strip, collapse internal whitespace, 64-char cap, dedupe) across all write paths. Spec: `docs/specs/tagging-spec.md` v2.
