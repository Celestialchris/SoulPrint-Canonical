...
- An existing Chatterbox virtualenv (e.g. `<path-to-VoiceForge>\.venv-chatter`) with Python 3.11, `chatterbox-tts`, `torch`, and `torchaudio` installed.
...
```powershell
<path-to-VoiceForge>\.venv-chatter\Scripts\pip.exe install -r reader\requirements.txt
```
...
```powershell
$env:READER_HOME = "<path-to-VoiceForge>"
<path-to-VoiceForge>\.venv-chatter\Scripts\python.exe -m uvicorn reader.backend.app:app --host 127.0.0.1 --port 5001
```
...
| --- | --- | --- |
| `READER_HOME` | `<path-to-VoiceForge>` | Root for refs and output. |
...
```powershell
<path-to-VoiceForge>\.venv-chatter\Scripts\python.exe -m pytest reader\tests\ -v
```
