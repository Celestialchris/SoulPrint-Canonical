...
**Export.** Memory Passport with manifest, canonical JSONL, provenance index, and checksums. Validate any passport against the contract. Conversations with file attachments export as attachment-aware `.zip` downloads, or as a `<stem>.assets/` sibling folder next to the markdown when exporting to a directory, with provenance manifests in either case. The archive is a file you own.
